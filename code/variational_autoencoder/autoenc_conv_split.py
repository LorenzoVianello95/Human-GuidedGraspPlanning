...
from util import *#load_data, extend_data, manipulate_depth
...
def plot_results(models,
    ...
    plt.imshow(x_gt[0,:,:,3]*10, cmap=plt.cm.gray_r)#, vmin=0.2, vmax=0.8)
    
    plt.subplot(num_col, num_row, 5)
    plt.imshow(x_result[0,:,:,3]*10, cmap=plt.cm.gray_r)#, vmin=0.2, vmax=0.8)
    
    plt.subplot(num_col, num_row, 6)
    plt.imshow(x_result_mean[0,:,:,3]*10, cmap=plt.cm.gray_r)#, vmin=0.2, vmax=0.8)
...
def transform_data(models,
                 data,
                 batch_size=128,
                 model_name="vae_grasps"):

    train_GP_set= "/home/lvianell/Desktop/Lorenzo_report/datasets/grasp_figures/paletta"
    test_GP_set= "/home/lvianell/Desktop/Lorenzo_report/datasets/grasp_figures/paletta/test_set"

    x_GP_train, y_GP_train, _, _ = load_GP_data(train_GP_set,1)
    x_GP_test, y_GP_test, _, _ = load_GP_data(test_GP_set,1)

    print("training GP shape")
    print(x_GP_train.shape, y_GP_train.shape)
    print("testing GP shape")
    print(x_GP_test.shape, y_GP_test.shape)

    x_GP_train = x_GP_train.reshape(x_GP_train.shape[0], 100, 100, 4).astype('float32')
    x_GP_test = x_GP_test.reshape(x_GP_test.shape[0], 100, 100, 4).astype('float32')

    x_GP_train= manipulate_depth(x_GP_train)  
    x_GP_test= manipulate_depth(x_GP_test) 
    
    encoder, decoder = models

    z_GP_train= encoder.predict(x_GP_train, batch_size=x_GP_train.shape[0])
    z_GP_test= encoder.predict(x_GP_test, batch_size=x_GP_test.shape[0])

    print("latent vectors shape:")
    print(z_GP_train.shape)
    print(z_GP_test.shape)

    x_GP_train_reconstructed = decoder.predict(z_GP_train, batch_size=batch_size)
    x_GP_test_reconstructed = decoder.predict(z_GP_test, batch_size=batch_size)

    x_gt= np.reshape(x_GP_train, [-1, 100,100,4])
    x_result= np.reshape(x_GP_train_reconstructed, [-1, 100,100,4] )

    fig = plt.figure()
    num_row=2
    num_col=2

    print("Difference in the color:", np.mean(np.abs(x_gt[:,:,:,0:3]- x_result[:,:,:,0:3])))
    print("Difference in depth:", np.mean(np.abs(x_gt[:,:,:,3]- x_result[:,:,:,3])))
...
x_train= manipulate_depth(x_train)       #MANIPOLO LA DEPTH ELIMINANDO VALORI TROPPO VICINI E TROPPO DISTANTI E NORMALIZZO
x_test= manipulate_depth(x_test)

#x_train= extend_data(x_train)
print("SSSSSSSSSSSSSSS",  x_train.shape)
...
"""
ss = ss.reshape(ss.shape[0], 100, 100, 4).astype('float32')
fig = plt.figure()

plt.subplot(2, 2, 1)
plt.title("GT ")
plt.imshow(cv2.cvtColor(x_train[0,:,:,0:3], cv2.COLOR_BGR2RGB))

plt.subplot(2, 2, 2)
plt.title("Decoder result")
plt.imshow(cv2.cvtColor(ss[0,:,:,0:3], cv2.COLOR_BGR2RGB))

plt.subplot(2, 2, 3)
plt.imshow(x_train[0,:,:,3], cmap=plt.cm.gray_r)#, vmin=0.2, vmax=0.8)

plt.subplot(2, 2, 4)
plt.imshow(ss[0,:,:,3], cmap=plt.cm.gray_r)#, vmin=0.2, vmax=0.8)
...
z_conv_RGB = Conv2D(32, (2, 2), activation='relu', padding="same", strides=2)(z_conv_RGB)
#z_conv_RGB = Conv2D(128, (2, 2), activation='relu', padding="same", strides=3)(z_conv_RGB)

lat_dim_RGB= 7*7*32
z_conv_RGB_reshaped= Reshape((lat_dim_RGB,), input_shape=(7,7,32))(z_conv_RGB)
z_conv_RGB_reshaped = Dense(1000, name= "dense_RGB")(z_conv_RGB_reshaped)
...
x_DEPTH = Conv2D(12, (3, 3), activation='relu', padding='same', strides=2)(x_DEPTH)
z_conv_DEPTH = Conv2D(16, (2, 2), activation='relu', padding="same", strides=2)(x_DEPTH)
z_conv_DEPTH = Conv2D(32, (2, 2), activation='relu', padding="same", strides=2)(z_conv_DEPTH)
#z_conv_DEPTH = Conv2D(128, (2, 2), activation='relu', padding="same", strides=3)(z_conv_DEPTH)

lat_dim_DEPTH= 7*7*32
z_conv_DEPTH_reshaped= Reshape((lat_dim_DEPTH,), input_shape=(7,7,32))(z_conv_DEPTH)
z_conv_DEPTH_reshaped= Dense(1000, name= "dense_DEPTH")(z_conv_DEPTH_reshaped)
...
FACTOR= 1.5
z_RGB = Dense(500, name= "reshaping_RGB")(z_conv_RGB_reshaped)
z_DEPTH = Dense(500, name= "reshaping_DEPTH")(z_conv_DEPTH_reshaped)
...
latent_space_dimension= 1000#lat_dim_RGB//FACTOR+lat_dim_DEPTH//FACTOR
...
#rgb= Dense(lat_dim_RGB, name= "dense_decoder_rgb")(rgb)

depth = Lambda(lambda x: x[:,lat_dim_RGB:])(d)
#depth= Dense(lat_dim_DEPTH, name= "dense_decoder_depth")(depth)

#decoder RGB ____________________________________________________________
x= Reshape((7,7,32), input_shape=(lat_dim_RGB,))(rgb)
...
x= Reshape((7,7,32), input_shape=(lat_dim_DEPTH,))(depth)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-t", "--transf", help=help_, action='store_true')
    ...
    if args.weights:
        ...
        if args.transf:
            print("Calculate Vectors for the dataset")
            transform_data(models,
                    data,
