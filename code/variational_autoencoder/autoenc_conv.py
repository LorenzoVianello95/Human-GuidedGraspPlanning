...
z = Conv2D(16, (2, 2), activation='relu', padding="same", strides=2)(x)

"""lat_dim= 13*13*16
z_mid= Reshape((lat_dim,), input_shape=(13,13,16))(z_conv)
z_mean = Dense(lat_dim//2, name='z_mean')(z_mid)
z_log_var = Dense(lat_dim//2, name='z_log_var')(z_mid)
z = Lambda(sampling, output_shape=(lat_dim//2,), name='z')([z_mean, z_log_var])
"""
...
x = Conv2D(16, (2, 2), activation='relu', padding="same")(latent_inputs)
...
if __name__ == '__main__':
    ...
    if args.mse:
        ...
        reconstruction_loss = binary_crossentropy(inputs,
                                                  outputs)
        sub=np.abs(inputs-outputs)*10       #aggiungo un valore ulteriore loss per la depth 
        add= K.mean(sub[:,:,:,3])
        print(np.shape(add))
